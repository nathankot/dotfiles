...
for SYM in `echo $SYMBOLS | tr ':' "\n"`; do
  RESULT=$(echo $RESULT | $JQ --arg n $SYM --arg t $TIME_FRAME '. + [{ symbol: $n, history: "", timeframe: $t }]')
  if [ "$SYM" != "BTC" ]
    then
      history=$(curl -A "$USER_AGENT" --fail -s "http://ichart.finance.yahoo.com/table.csv?s=${SYM}%3DX&g=d&ignore=.csv&c=${Y}&a=$(($M - 1))&b=${D}" \
            ...
            | awk -F, '{ print $4 * 100 }' \
            | $SPARK)
      RESULT=$(echo $RESULT | $JQ --arg n $SYM --arg h $history '(.[] | select(.symbol==$n) | .history) = $h')
    fi

  PRICE=$(curl -A "$USER_AGENT" --fail -s http://finance.yahoo.com/webservice/v1/symbols/${SYM}USD%3DX/quote?format=json \
            | $JQ -c '.list.resources[0].resource.fields.price | tonumber')
  RESULT=$(echo $RESULT | $JQ --arg p "$PRICE" --arg n "$SYM" '(.[] | select(.symbol==$n) | .price) = $p')
done
